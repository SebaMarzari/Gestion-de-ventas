...
class MyApp(App):
    ...
    def ventas(self):
        ...
        caja.Add(boton)
        relleno = StaticText(self.panel3) #El relleno espara qe el botòn Buscar se ubique en el lado opuesto del botòn Agregar
        caja.Add(relleno, 4, flag = EXPAND)
        self.searchV = search = SearchCtrl(self.panel3) #Boton buscar
        search.ShowCancelButton(True)
        ...
        try:
            cur .execute("CREATE TABLE Datosv (Id INTEGER PRIMARY kEY, Producto NVARCHAR, Costo NVARCHAR, Cantidad NVARCHAR, Fecha NVARCHAR)")
        except:
            pass

        self.searchV.Bind(EVT_TEXT, self.buscarV)
    ...
    def vender(self, event):
        ...
        self.listaVentas = listaVentas = ["Nombre y Apellido","DNI", "Telefono", "Producto", ["Liquidos", "Equipos"], "Cantidad", "Precio"]
        j = 1
        for i in listaVentas :
            if j == 5 :
                ...
                columStatick.Add(aux, 1, ALL, 22)
            j += 1
        self.nombreC = nombreC = TextCtrl(self.fVenta, size = (150, 35))
        columOpcionesVentas.Add(nombreC, 1, ALL, 10)
        self.dniC = dniC = TextCtrl(self.fVenta, size = (100, 35), validator = MyValidator())
        dniC.SetMaxLength(9)
        columOpcionesVentas.Add(dniC, 1, ALL, 10)
        self.tel = tel = TextCtrl(self.fVenta, validator = MyValidator(),size = (100, 40))
        columOpcionesVentas.Add(tel, 1, ALL, 10)
        self.aux3 = aux3 = RadioBox (self.fVenta, -1 , choices = listaVentas[4], style = NO_BORDER | RA_SPECIFY_COLS)
        columOpcionesVentas.Add(aux3, 1, LEFT, 10)
        aux3.Bind(EVT_RADIOBOX, self.productos)
        self.eleccionP = eleccionP = ComboBox (self.fVenta, -1 , choices = listaAux ,size = (140, 40))
        columOpcionesVentas.Add(eleccionP, 1 , ALL, 10)
        self.cantP = cantP = TextCtrl (self.fVenta, validator = MyValidator(),size = (100, 40))
        columOpcionesVentas.Add(cantP, 1, ALL, 10)
        self.precioP = precioP = TextCtrl(self.fVenta, validator = MyValidator(),size = (100, 40))
    ...
    def agregarACarro(self, event):
        ...
        self.condicionR = self.veoRestaStokL(tipo, idp, cantidad)
        if self.condicionR == True:
            listaCarro.append(str(nombreProducto))
            self.listaCompras.append(str(nombreProducto))
            ...
            self.tipoR.append(tipo)
            self.cantidadR.append(cantidad)
            self.idpR.append(idp)
            self.productosInfo += listaCarro[0] + "#"
            self.cantidadInfo += listaCarro[1] + "#"
            self.precioInfo += listaCarro[2] + "#"
        else:
            self.cartelError("Error no se puede agregar producto a la lista")
    ...
    def finalizarLaCompra(self, event):
        ...
        self.comparadoridV += 1
        self.listaCompras.append(str(self.comparadoridV))
        self.listaCompras.append(nombreCliente)
        ...
        fechaCompleta = time.strftime("%d"+ "/" + "%m" + "/" + "%Y")
        self.grabaBDInfoVentas(self.productosInfo, self.precioInfo, self.cantidadInfo, fechaCompleta)
        self.productosInfo = ""
        self.precioInfo = ""
        self.cantidadInfo = ""
        print("Lista de compras", self.listaCompras)
        for p in range(len(self.idpR)):
            self.restaStokL(str(self.tipoR[p]), int(self.idpR[p]), int(self.cantidadR[p]))
            print(self.tipoR[p])
            print(self.idpR[p])
            print(self.cantidadR[p])
    ...
    def cliente (self, e):
        ...
        cantidad = ""
        fecha = ""
        v = Frame(None, title = "Información del cliente", size=(600,200))
        dvlc = DataViewListCtrl(v)
        row = int(self.grillaV.GetSelectedRow())

        idc = int(self.grillaV.GetTextValue(row, 0))
        monto = self.grillaV.GetTextValue(row, 1)
        encabezado = ["Producto", "Costo", "Cantidad", "Fecha de compra"]
        for i in encabezado :
            dvlc.AppendTextColumn (i, width = 130)
        lista = self.recupBDInfoVentas()
        ...
        productosListos = productos.split("#")
        preciosListos = precio.split("#")
        cantidadListos = cantidad.split("#")
        for h in range(len(productosListos)):
            if productosListos[h] != "":
                dvlc.AppendItem([str(productosListos[h]), str(preciosListos[h]), str(cantidadListos[h]), str(fecha)])

        print (productosListos)
        print(preciosListos)
        print(cantidadListos)
        print(fecha)
    ...
    def recupBDInfoVentas(self):
        con = sqlite3.connect("VaperOfice.db")
        cur = con.cursor()
        cur.execute("SELECT * FROM Datosv ORDER BY id")
        tuplas = cur.fetchall()
        listaA = [list(e) for e in tuplas]
        con.close()
        return listaA
    ...
    def agregarEquipos (self, event):
        self.fEquipos = Frame (None, title = "Agregar producto", size = (350,410))
        ...
        for i in self.encabezado2 :
            ...
            if j == 1 or j == 5 :
                ...
                columOpciones.Add(aux3, 1, ALL, 10)
                k += 1
            j += 1
        botonAgregar = Button (self.fEquipos, -1, label = "Agregar", size = (100, 35))
        botonCerrar = Button (self.fEquipos, -1 ,label = "Cerrar", size = (100, 35))
        botonAgregar.Bind(EVT_BUTTON, self.AgregarE)
        botonCerrar.Bind(EVT_BUTTON, self.CerrarE)
        columOpciones.Add(botonCerrar, 1, ALL, 10)
    ...
    def agregarLiquidos (self, event):
        self.ff = Frame (None, title = "Agregar producto", size = (350,500))
        ...
        for i in self.encabezado :
            ...
            columStatick.Add(aux, 1, ALL, 22)
            if j == 1:
                aux2 = TextCtrl(self.ff, size = (100,30))
                self.lista.append(aux2)
                columOpciones.Add(aux2, 1, ALL, 9)
            elif  j == 4:
                aux2 = TextCtrl(self.ff,size = (100,30) , validator = MyValidator() )
                self.lista.append(aux2)
                columOpciones.Add(aux2, 1, ALL, 8)
            else:
                ...
                k += 1
            j += 1
        botonAgregar = Button (self.ff, -1, label = "Agregar", size = (100, 35))
        botonCerrar = Button (self.ff, 1 ,label = "Cerrar", size = (100, 35))
    ...
    def grabaBDInfoVentas(self, iproducto, icosto, icantidad, ifecha):
        engine = create_engine('sqlite:///VaperOfice.db')
        conn = engine.connect()
        metadata = MetaData()

        lInfoVentas = Table('Datosv', metadata, 
            Column('Id', Integer(), primary_key=True),
            Column('Producto', String(200)),
            Column('Costo', String(200)),
            Column('Cantidad', String(200)),
            Column('Fecha', String(200)),
            )

        t = conn.begin()
        ins = lInfoVentas.insert().values(Producto = iproducto, Costo = icosto, Cantidad = icantidad, Fecha = ifecha)    
    ...
    def veoRestaStokL(self, producto, idl, cantARestar):
        condi = False
        listaLiquidos = self.recupBDLiquidos()
        listaEquipos = self.recupBDEquipos()
        valorAnterior = 0
        if producto == "liquidos":
            print("Entre liquidos, viendo si resto...")
            for e in listaLiquidos:
                valorAnterior = int(e[4])
                print ("ValorAnterior viendo si resto..:", valorAnterior)
                print(e[0])
                if int(e[0]) == idl :
                    print("Encontre el producto: " + e[1] + "Viendo si resto")
                    e[4] = int(e[4]) - cantARestar
                    e[4] = int(e[4])
                    print("Reste de la base de datos", e[4], " Reste")
                    if e[4] < 0 :
                        e[4] = valorAnterior
                        self.cartelStock("Esta restando mas del producto que posee en Stock o no posee mas stock del producto")
                        condi = False
                    elif e[4] <= 20 and e[4] >= 1:
                        e[4] = str(e[4])
        ...
        if producto == "liquidos":
            ...
            for e in listaLiquidos:
                ...
                if int(e[0]) == idl :
                    ...
                    print ("No encontre nada liquidos")
        elif producto == "equipos":
            for e in listaEquipos:
                valorAnterior = int(e[5])
                if int(e[0]) == idl :
                    ...
                    if e[5] < 0 :
                     ...
                     print ("No encontre nada equipos")
...
## Comienza funciones Buscar

    def buscar(self, event):
        lista = self.recupBDLiquidos()
        print("--------------------------")
        busco = str(self.search.GetValue())
        busco = busco.upper()
    ...
    def buscarV (self, e):
        lista = self.recupBDVentas()
        print("--------------------------")
        busco = self.searchV.GetValue() 
        busco = busco.upper()
        num = self.searchV.GetLastPosition()
        tot = self.grillaV.GetItemCount()
        for i in range(tot):
            self.grillaV.DeleteItem(0)

        for i in range(len(lista)):
            if busco == lista[i][1][:num].upper():
                self.grillaV.AppendItem(lista[i])
                print("#", lista[i][1])

## Terminan funciones Borrar
